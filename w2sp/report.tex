...
The two descriptions are targeting two different audiences: one with much more knowledge about technology and Tor, the other with much less knowledge of these topics.  It is likely that at least one of these audiences will not understand one of the descriptions.  This could be a problem if users simply decide not to do anything with that setting (i.e. don't change the setting to use the Safeplug as a relay node).  In this case, there would be a large increase in use of the Tor network, yet most of the users are not giving back to it.
